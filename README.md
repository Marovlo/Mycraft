...
### 👾 生物与 AI
- 被动生物（猪、牛、羊、鸡）、敌对生物（僵尸、骷髅、苦力怕、蜘蛛）
- A* 寻路 + 多目标 AI 系统
- 昔夜循环 + 夜晚怪物生成
- 骷髅拉弓动画 + 蓄力射箭

### 🌐 联机系统
- 单人模式（内置服务器）+ 多人模式（专用服务器）
- 基于 ENet 的 UDP 网络层（可靠 + 不可靠通道）
- 远程玩家第三人称 Steve 模型渲染
- 远程玩家动作同步（挥臂、拉弓、吃东西、潜行）
- F5 视角切换（第一人称 / 第三人称背面 / 第三人称正面）
...
```bash
./mycraft.sh                    # 编译 (Release) 并运行（默认）
./mycraft.sh setup              # 安装依赖
./mycraft.sh build              # 仅编译
./mycraft.sh build --debug      # Debug 编译
./mycraft.sh build --clean      # 清理后重新编译
./mycraft.sh build --jobs 8     # 指定并行编译线程数
./mycraft.sh run                # 仅运行（跳过编译）
./mycraft.sh run --vlog         # 运行并启用调试日志
./mycraft.sh start --debug      # Debug 编译并运行
./mycraft.sh server             # 编译并运行专用服务器
./mycraft.sh server --build     # 仅编译服务器
./mycraft.sh help               # 查看帮助
```

> 注：旧的 `setup.sh`、`build.sh`、`run.sh` 仍然可用，但推荐使用统一的 `mycraft.sh`。
...
- [x] **Phase 5** — 生物与 AI（被动/敌对生物、寻路、战斗、昼夜循环、弓箭系统）
- [ ] **Phase 6** — 联机系统（ENet UDP 网络、内置服务器、专用服务器、玩家同步）
